(() => {
  ...
  function loadSelectedFront() {
    const name = getSelectedCharacterName();
    const file = `${name}_front.png`;
    if (SELECTED_FRONT.name === name && SELECTED_FRONT.image) return Promise.resolve();
    return new Promise((resolve) => {
      const im = new Image();
      im.onload = () => { SELECTED_FRONT.name = name; SELECTED_FRONT.image = im; resolve(); };
      im.onerror = () => { SELECTED_FRONT.name = name; SELECTED_FRONT.image = null; resolve(); };
  ...
  function renderRoomBase() {
    ...
    if (state === "intro") {
      ...
      if (t < 1) {
        ...
        ctx.drawImage(IMG["character_front.png"], fx, layout.character.y, fw, layout.character.h);
