(() => {
  ...
  const AUDIO = {
    ...
    playCompleteSong() {
      const src = this.completeSongSrc;
      if (!src || this.completePlayed) return;
      this.stopBgm();
    ...
    playFailSong() {
      const src = this.failSongSrc;
      if (!src || this.failPlayed) return;
      this.stopBgm();
  ...
  function getSelectedFrontFilename() {
    return `${getSelectedCharacterName()}_front.png`;
  }
  ...
  function loadImageWithFallback(primaryName, fallbackName) {
    return new Promise((resolve) => {
      ...
      im.onerror = () => {
        ...
        fallback.src = "assets/img/" + fallbackName;
      };
      im.src = "assets/img/" + primaryName;
    });
  }

  async function loadSelectedFront() {
    const name = getSelectedCharacterName();
    const file = `${name}_front.png`;
    if (SELECTED_FRONT.name === name && SELECTED_FRONT.image) return;
    const im = await loadImageWithFallback(file, "character_front.png");
    SELECTED_FRONT.name = name;
    SELECTED_FRONT.image = im;
  }
  ...
  async function loadSelectedCharacterAssets() {
    ...
    const [front, backIdle, backLeft, backRight] = await Promise.all([
      loadImageWithFallback(`${name}_front.png`, "character_front.png"),
      loadImageWithFallback(`${name}_back_idle.png`, "character_back_idle.png"),
      loadImageWithFallback(`${name}_back_left.png`, "character_back_left.png"),
      loadImageWithFallback(`${name}_back_right.png`, "character_back_right.png")
    ]);

    SELECTED_CHARACTER_ASSETS.name = name;
    SELECTED_CHARACTER_ASSETS.front = front || IMG["character_front.png"] || null;
    SELECTED_CHARACTER_ASSETS.backIdle = backIdle || IMG["character_back_idle.png"] || null;
    SELECTED_CHARACTER_ASSETS.backLeft = backLeft || IMG["character_back_left.png"] || null;
    SELECTED_CHARACTER_ASSETS.backRight = backRight || IMG["character_back_right.png"] || null;
  }
    ...
    return Promise.all([
      ...toLoad.map(name => new Promise((res) => {
        ...
        im.onerror = () => {
          ...
          // No bloquegem el flux per assets de personatge personalitzats que puguin faltar.
          // Fem servir un placeholder transparent perquè el joc pugui arribar al menú de dificultat.
